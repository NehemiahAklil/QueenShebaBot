    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Failed to create federation! Head over to @PhoenixSupport to notify us of the error.")
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem persists, reach out to us @PhoenixSupport.")
            return
...
def unfban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text(f"{mention_html(user_chat.id, user_chat.first_name)} has been un-fbanned.",
                       parse_mode=ParseMode.HTML)
    FEDADMIN = sql.all_fed_users(fed_id)
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Failed to set federation rules. If this persists, reach out to us @PhoenixSupport.")
            return
...
def fed_ban_list(bot: Bot, update: Update, args: List[str], chat_data):
    ...
    getfban = sql.get_all_fban_users(fed_id)
    if len(getfban) == 0:
        update.effective_message.reply_text(f"The federation ban list of {info['fname']} is empty.",
                                            parse_mode=ParseMode.HTML)
        return
    ...
    if args:
        if args[0] == 'json':
            ...
            for users in getfban:
                ...
                json_parser = {"user_id": users, "first_name": getuserinfo['first_name'],
                               "last_name": getuserinfo['last_name'], "user_name": getuserinfo['user_name'],
            ...
            for users in getfban:
                getuserinfo = sql.get_all_fban_users_target(fed_id, users)
                user_id = users, first_name = getuserinfo['first_name'], last_name = getuserinfo[
                                                                                         'last_name'], user_name = \
                    getuserinfo['user_name'], reason = getuserinfo['reason']
...
def fed_chats(bot: Bot, update: Update, args: List[str]):
    ...
    if len(getlist) == 0:
        update.effective_message.reply_text(f"No users are fbanned from the federation {info['fname']}",
                                            parse_mode=ParseMode.HTML)
        return
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if int(import_userid) in WHITELIST_USERS:
                        failed += 1
                        continue
                    addtodb = sql.fban_user(fed_id, str(import_userid), import_firstname, import_lastname,
    ...
    if getfed:
        ...
        if delete:
            query.message.edit_text(
                f"You have deleted your federation! Now all the groups that were connected with `{getfed['fname']}` do not have a federation.",
                parse_mode='markdown')
